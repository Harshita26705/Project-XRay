...
# Project X-Ray — Execution Plan and Cost-Aware Adjustments

This section captures the actual decisions made from the current state and constraints.

## Verified repo state
- Repository is empty except for `prompt.md`.
- There is no existing app stack or code to preserve.

## Verified Microsoft SDK facts
- `azure-ai-projects` is the Microsoft Foundry Python SDK and requires a Foundry project endpoint in the form:
  `https://<account>.services.ai.azure.com/api/projects/<project>`
- Authentication is Entra ID only, using `DefaultAzureCredential`.
- The recommended usage is:
  `AIProjectClient(endpoint=..., credential=DefaultAzureCredential())`
- The SDK supports `project_client.get_openai_client()` and `openai_client.responses.create(...)`.
- Azure AI Search uses `azure-search-documents` and supports hybrid search through both keyword + vector queries.

## Key user decisions
- Build start: 3–5 day hackathon sprint.
- Cost minimization: prefer local demo, SQLite, no Azure compute, no paid webhooks, no Azure SQL unless necessary.
- Platform stack: Python + FastAPI backend.
- Demo app language: C#/.NET backend + React/TypeScript frontend, but the platform parser should also handle Python projects.
- Persistence: SQLite + in-memory NetworkX graph.
- Local hosting only for the demo: local server + dev tunnel / ngrok for external webhooks.
- Read-only database context: local SQL schema / SQLite in demo mode. Azure SQL is a stretch goal.
- AI service: user has no Azure AI resources yet. Build an adapter that supports Foundry, Azure OpenAI, and mock mode.
- Power Automate: free licence only. Prefer direct Azure DevOps hook → X-Ray API + Teams Webhook path. Treat Power Automate as should-have / stretch.

## Execution priority changes
Because the repo is empty and resources are not yet created, the most reliable MVP path is:
1. Build the controlled demo app.
2. Build the deterministic graph and blast-radius engines first.
3. Add AI reasoning adaptively and behind an adapter.
4. Add Azure DevOps and Search only after the core loop works.
5. Add Teams and Power Automate only after the core product is validated.

## MUST HAVE for MVP
- Local demo app with known dependency chain
- FastAPI backend with health and analysis endpoints
- Parser + graph + traversal
- Deterministic blast-radius algorithm
- Risk engine
- AI adapter with mock/fallback mode
- Security scanner normalization + degraded mode
- UI dashboard with GREEN/YELLOW/RED graph
- End-to-end analysis flow for ticket and PR scenarios

## SHOULD HAVE
- Azure AI Search knowledge index
- Azure DevOps adapter and webhook flow
- Reports
- PR comments
- Teams notifications
...
- Read-only database metadata integration
- Sandbox verification
- Azure deployment
- Semgrep or other heavier SAST integration

## Implementation principle
Keep the architecture simple enough to finish in a hackathon: graph-first, deterministic-by-default, AI as an explanation layer, and every external dependency isolated behind an adapter.

---

# Final instruction to the agent

Do not implement any files yet.

Before each phase:
1. inspect the repository state
2. confirm whether a project already exists
3. list relevant files
4. identify the current stack
5. compare with the architecture above
6. propose phase-specific files and dependencies
7. wait for confirmation before writing code
